...
/*   Updated: 2018/02/13 15:11:15 by jonkim           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (keycode == F_NEXT || keycode == F_PREV)
	{
		keycode == F_NEXT ? env->fractal++ : env->fractal--;
		if (env->fractal == 10)
			env->fractal = 1;
		else if (env->fractal == 0)
			env->fractal = 9;
	}
...
int		ft_window_hook(int keycode, t_env *env)
{
	...
	if ((keycode >= 123 && keycode <= 126) || keycode == 24 || keycode == 27
			|| keycode == 15 || keycode == 49)
		ft_window_mvmt(keycode, env);
